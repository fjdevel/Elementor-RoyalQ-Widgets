 ...
 * @since 1.0.0
 */
class Elementor_Huobi_Royal extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Huobi Button', 'elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_icon',
			[
				'label' => esc_html__( 'Houbi', 'elementor' ),
			]
		);
	...
	protected function render() {
        ...
        if(!empty($username)){
            ...
            $sql =  "SELECT meta_value FROM wp_usermeta WHERE meta_key = 'huobi_link' AND user_id =".$user->ID;
